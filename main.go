...
func errorsRoute(w http.ResponseWriter, r *http.Request) {
	start := time.Now()
	log.Print("An intentional error occurred. %s", "path", r.URL.Path)
	duration := time.Since(start)
...
func main() {
	...
	// Simulate an error
	http.Handle("/error", otelhttp.NewHandler(http.HandlerFunc(errorsRoute), "ErrorsHandler"))
